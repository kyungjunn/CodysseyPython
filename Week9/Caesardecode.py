...
WORD_DICTIONARY = [
    ...
    'base', 'mars', 'password', 'secret', 'open', 'door',
    'emergency', 'access', 'key', 'code', 'storage', 'unlock'
]
...
def contains_dictionary_word(text):
    """텍스트 내에 사전 단어가 포함되어 있는지 확인한다. (보너스 과제)"""
    ...
    for shift in range(1, alphabet_count + 1):
        ...
        for char in target_text:
            if char.isalpha():
                ...
                # 알파벳 범위 내에서 shift 만큼 역방향 이동
                decoded_char = chr((ord(char) - base - shift) % alphabet_count + base)
        ...
        # 보너스 과제: 사전 단어 감지 여부 확인
        found, matched_word = contains_dictionary_word(decoded_text)
        marker = f'  ★ 사전 단어 발견: "{matched_word}"' if found else ''

        print(f'  Shift {shift:>2}자리 → {decoded_text}{marker}')

        # 보너스 과제: 사전 단어가 발견되면 반복 중단
        if found:
            print(f'\n  [자동 감지] Shift {shift}에서 의미 있는 단어 발견! 반복을 중단합니다.')
            break
...
def main():
    ...
    print(f'\n  [선택 결과] Shift {correct_shift}자리 → 최종 해독 암호: "{final_decoded}"')
